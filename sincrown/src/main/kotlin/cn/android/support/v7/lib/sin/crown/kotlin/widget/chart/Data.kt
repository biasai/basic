 ...
 * 图表数据
 */
class Data(var positionX: Float = 0f) {//positionX X轴开始下标，默认就是从0,原点开始计算。一般都是从1开始的。
    ...
    fun put(x2: ArrayList<String>, y2: FloatArray): Data {
     ...
     * 根据下标，获取X轴数据
     */
    fun getX(position: Float): String? {
     ...
     * 根据X轴坐标获取Y轴下标
     */
    fun getY(positionX: Float): Float? {
     ...
     * 回调，获取所有的x轴下标和y轴下标
     */
    fun getXY(xy: (xPosition: Float, yPosition: Float) -> Unit) {
//        mapX?.forEach { xs, xf ->
//            var yf = mapY.get(xs)
//            yf?.let {
//                xy(xf, yf / unitY)
