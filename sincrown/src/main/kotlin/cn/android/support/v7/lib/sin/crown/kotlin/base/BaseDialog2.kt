...
abstract class BaseDialog2(open var activity: Activity?, open var layoutId: Int = 0, open var isStatus: Boolean = true, open var isTransparent: Boolean = true, open var isInitUI: Boolean = false) {
    ...
    fun isDarkmode(isDarkmode: Boolean = BaseApplication.getInstance().darkmode): BaseDialog2 {
        BaseApplication.getInstance().setStatusBarDrak(dialog!!.window, isDarkmode)
        return this
    }

    //fixme 触摸最外层控件，弹窗是否消失。
    fun isDismiss(isDismiss: Boolean = true): BaseDialog2 {
        //触摸最外层控件，是否消失\
        dialog?.window.let {
            if (isDismiss) {
    ...
    fun isLocked(isLocked: Boolean = true, callback: (() -> Unit)? = null): BaseDialog2 {
    ...
    init {
        //fixme 是否在父类中初始化视图，默认就是否，false。需要在子类中手动调用initUI()方法
        //fixme 如果为false,不在父类中初始化视图，则在子类的init方法中，必须手动调用initUI()方法。
        //fixme 之所以引用isInitUI参数，是因为子类的构造参数无法在父类中获取(父类无法获取子类的构造参数)。为了获取子类构造参数。则必须在子类中初始化。
        //fixme initUI()在子类中调用时，onCreateView()就可以获取子类构造参数了。
        if (isInitUI) {
            initUI()
        }
    }
    ...
    //显示监听
    fun onListener(onListener: () -> Unit): BaseDialog2 {
    ...
    //消失监听
    fun onRecycleView(onRecycleView: () -> Unit): BaseDialog2 {
