...
//    }
    ...
    //显示成功文本,浅绿色背景
    open fun showSuccess(text: String?, init: ((textView: RoundTextView) -> Unit)? = null) {
        show(text) {
            it.apply {
                backgroundColor = Color.parseColor("#61A465")
                leftPadding = px.x(80)
                rightPadding = px.x(30)
                //画正确的勾勾
                draw { canvas, paint ->
                    paint.color = Color.WHITE
                    paint.strokeCap = Paint.Cap.ROUND
                    paint.style = Paint.Style.STROKE
                    paint.strokeWidth = px.x(4.5f)
                    var w = px.x(30f)
    ...
    //显示失败文本,浅红色背景
    open fun showError(text: String?, init: ((textView: RoundTextView) -> Unit)? = null) {
        show(text) {
            it.apply {
    ...
    //显示提示信息，浅蓝色背景
    open fun showInfo(text: String?, init: ((textView: RoundTextView) -> Unit)? = null) {
        show(text) {
            it.apply {
                ...
                draw { canvas, paint ->
                    ...
                    var r = px.x(5f) / 2
                    canvas.drawCircle(startX + w / 2, startY + r + px.x(5f), r, paint)//点
                    paint.style = Paint.Style.STROKE
                    paint.strokeWidth = px.x(3.5f)
    ...
    //显示一般文本,浅黑色背景
    open fun show(text: String?, init: ((textView: RoundTextView) -> Unit)? = null) {
        text?.let {
            if (it.trim().equals("")) {
                return
            }
