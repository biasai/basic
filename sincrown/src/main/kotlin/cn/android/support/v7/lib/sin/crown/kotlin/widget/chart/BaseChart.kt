 ...
 * 宽高，位置。必须和X轴，Y轴一致。
 */
open abstract class BaseChart : BaseView {
    ...
    fun setAxis(xAxis: XAxis, yAxis: YAxis) {
        this.xAxis = xAxis
        this.yAxis = yAxis
    }
    ...
    fun setData(data: Data): BaseChart {
        this.data = data
        invalidate()
        return this
    }
    ...
    //设置X轴，Y轴，和数据
    fun set(xAxis: XAxis, yAxis: YAxis, data: Data) {
        setAxis(xAxis, yAxis)
        data(data)
    }
     ...
     * 绘制数据点文本案例
     */
    fun drawPointText(canvas: Canvas, paint: Paint, x: Float, y: Float, text: String, sub: Int) {
        when (sub) {
            sub_big -> {
     ...
     * value Y轴数据
     */
    fun getStu(position: Float, value: Float): Int {
        var sub = sub_big
        data?.let {
     ...
     * value Y轴数据
     */
    fun getXY(xy: (x: Float, y: Float, position: Float, value: Float, sub: Int) -> Unit) {
        data?.let {
     ...
     * 获取第一个和最后一个坐标
     */
    fun getStartAndEndXY(xy: (startX: Float, startY: Float, endX: Float, endY: Float) -> Unit) {
        data?.let {
            var startX = 0f
            var startY = 0f
            var endX = 0f
     ...
     * 获取最高点和最低点
     */
    fun getMinAndMaxXY(xy: (minX: Float, minY: Float, maxX: Float, maxY: Float) -> Unit) {
        data?.let {
            var minX = 0f
            var minY = 0f
            var maxX = 0f
    ...
    fun drawPoint(drawPoint: (canvas: Canvas, paint: Paint, x: Float, y: Float, position: Float, value: Float, sub: Int) -> Unit) {
