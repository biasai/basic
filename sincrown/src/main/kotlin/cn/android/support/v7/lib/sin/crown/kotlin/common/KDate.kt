 ...
 *
 */
class KDate(var format: String = "yyyy-MM-dd HH:mm", var date: String? = null) {
    var calendar: Calendar
    var lunar: Lunar//fixme 农历时间,直接调用 kDate.lunar 就返回农历时间，如1992年十二月廿十。默认就调用了toString()方法。

    init {
        if (date == null) {
            calendar = Calendar.getInstance()//获取当前时间，一旦获取。Calendar就锁定了。即时间就停止在那一刻了，不会再变了。即每个Calendar的时间都是固定不变的。是锁定的。
        } else {
            calendar = TimeUtils.getInstance().getCalendar(date, format)//自定义时间
        }
        date = SimpleDateFormat(format).format(calendar.time)//获取当前格式的时间
        lunar = Lunar(calendar)//fixme 阳历转阴历
    }

    //获取指定格式的时间
    fun date(format: String = this.format): String {
    ...
    var AGE: Int = TimeUtils.getInstance().getAge(calendar)
     ...
     * 返回：大于0，表示当前calendar日期大于比较日期
     */
    fun DISTANCE_HOUR(compare: Calendar = Calendar.getInstance()): Long {
        var from: Long = calendar.time.time
        var to: Long = compare.time.time
        return ((from - to) / (1000 * 60 * 60))//当前calendar时间 减去 比较时间。
    }

    fun DISTANCE_HOUR(compare: KDate): Long {
     ...
     * 返回：大于0，表示当前calendar日期大于比较日期
     */
    fun DISTANCE_DAY(compare: Calendar = Calendar.getInstance()): Long {
        var from: Long = calendar.time.time
        var to: Long = compare.time.time
        return ((from - to) / (1000 * 60 * 60 * 24))//当前calendar时间 减去 比较时间。
    }

    fun DISTANCE_DAY(compare: KDate): Long {
